(function ($) {
	...
	if ($('.project-slider').length) {
		const projects_slider = new Swiper('.project-slider .swiper', {
			slidesPerView : 1,
			speed: 1000,
			spaceBetween: 30,
			loop: true,
	...
	/* World Map Card Elite Active JS Start */ 
	if ($('.world-map-card-btn-elite').length) {
			$('.world-map-card-btn-elite').on('click', function () {
				var $cardItem = $(this).closest('.world-map-card-item-elite');
				$('.world-map-card-item-elite').removeClass('active');
				$cardItem.addClass('active');
			});
	}
	/* World Map Card Active JS End */
	
	/* How Work List Item List Start */
	var $how_work_list_prime = $('.how-work-list-prime');
	if ($how_work_list_prime.length) {
		var $how_work_item_prime = $how_work_list_prime.find('.how-work-item-prime');

		if ($how_work_item_prime.length) {
			$how_work_item_prime.on({
				mouseenter: function () {
					if (!$(this).hasClass('active')) {
						$how_work_item_prime.removeClass('active'); 
